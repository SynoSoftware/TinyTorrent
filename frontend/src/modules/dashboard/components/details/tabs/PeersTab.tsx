...
export const PeersTab = ({
    ...
    const clampPeerContextMenuPosition = useCallback((x: number, y: number) => {
        const rect = listRef.current?.getBoundingClientRect();
        if (!rect) {
            return { x, y };
        }
        const maxX = Math.max(
            rect.width - CONTEXT_MENU_WIDTH - CONTEXT_MENU_MARGIN,
            CONTEXT_MENU_MARGIN
        );
        const maxY = Math.max(
            rect.height - CONTEXT_MENU_MARGIN,
            CONTEXT_MENU_MARGIN
        );
        return {
    ...
    const handlePeerContextAction = useCallback(
        (action: PeerContextAction) => {
            ...
            if (action === "copy_ip" && typeof navigator !== "undefined") {
                navigator.clipboard?.writeText(peerContextMenu.peer.address).catch(() => null);
            }
