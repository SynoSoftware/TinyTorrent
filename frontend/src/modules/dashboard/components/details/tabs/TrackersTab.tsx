    ...
    return (
        <div className="flex flex-col gap-3">
            {trackers.map((tracker) => {
                const trackerKey = tracker.id ?? `${tracker.announce}-${tracker.tier}`;
                    ...
                    >
                        <div className="flex items-start justify-between gap-4">
                            <div className="flex min-w-0 items-center gap-3">
                                <span
                                    className={`h-2.5 w-2.5 rounded-full shadow ${tracker.lastAnnounceSucceeded ? "bg-success" : "bg-warning"}`}
                                />
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs uppercase tracking-[0.35em] text-foreground/60">
                                        {t("torrent_modal.trackers.tier")} {tracker.tier}
                                    </p>
                                </div>
                            </div>
