...
import { Button, Textarea, Tooltip, cn } from "@heroui/react";
...
const TRACKER_UI = {
    selectionSummary: DETAILS.table.tableHeadRow,
    scroll: `${DETAILS.table.scroll} relative outline-none`,
    table: `${DETAILS.table.table} table-fixed`,
    headerCell:
        `${DETAILS.table.tableHeadCell} ${SURFACE.chrome.sticky} top-0 z-sticky bg-content1/80 backdrop-blur-sm`,
    headerButton:
        "flex w-full items-center gap-tight text-left text-inherit transition-colors hover:text-foreground/80",
    headerButtonEnd: "justify-end",
    row: `${DETAILS.table.tableRow} cursor-default`,
    rowSelected:
        "surface-layer-1 outline outline-1 -outline-offset-1 outline-primary/20",
    ...
    tierBadge:
        `${SURFACE.atom.insetRounded} inline-flex min-w-0 items-center px-tight py-tight text-label font-semibold text-foreground/65`,
...
const TrackerRow = ({
    ...
    <tr
        ...
        onClick={(event) =>
            viewModel.actions.handleRowClick(event, row.key, row.index)
        }
        onContextMenu={(event) =>
            viewModel.actions.openContextMenu(event, row.key, row.index)
        }
    >
        ...
        <td className={cn(TRACKER_UI.bodyCell, TRACKER_UI.metricCell)}>
            {row.seedsLabel}
        </td>
        <td className={cn(TRACKER_UI.bodyCell, TRACKER_UI.metricCell)}>
            {row.leechesLabel}
        ...
        <td className={cn(TRACKER_UI.bodyCell, TRACKER_UI.timeCell)}>
            <Tooltip
                content={row.lastAnnounceTooltip}
                classNames={SURFACE.tooltip}
                delay={500}
            >
        ...
        <td className={cn(TRACKER_UI.bodyCell, TRACKER_UI.timeCell)}>
            <Tooltip
                content={row.nextAnnounceTooltip}
                classNames={SURFACE.tooltip}
                delay={500}
            >
...
export const TrackersTab = ({
    ...
    const shell = (content: ReactNode) =>
        isStandalone ? (
            <GlassPanel className={DETAILS.table.panel}>{content}</GlassPanel>
        ) : (
            content
        );
            ...
            <div className={DETAILS.table.toolbar}>
                <div className={DETAILS.table.toolbarGroup}>
                    <Button
                        ...
                        startContent={<Plus />}
                        onPress={viewModel.actions.openAddModal}
                        isDisabled={viewModel.state.isMutating || torrentId == null}
                    >
                        {viewModel.labels.addLabel}
                    </Button>
                    <Button
                        ...
                        startContent={<Trash2 />}
                        onPress={() => {
                            void viewModel.actions.removeSelected();
                        }}
                        isDisabled={!viewModel.state.canRemove || viewModel.state.isMutating}
                    >
                        {viewModel.state.selectedCount > 1
                            ? viewModel.labels.removeManyLabel
                            : viewModel.labels.removeLabel}
                    </Button>
                    <Button
                        variant="flat"
                        startContent={<RefreshCcw />}
                        onPress={() => {
                            void viewModel.actions.reannounceTorrent();
                        }}
                        isDisabled={viewModel.state.isMutating || torrentId == null}
                    >
                        {viewModel.labels.reannounceLabel}
                    </Button>
                    <Button
                        variant="flat"
                        startContent={<Copy />}
                        onPress={() => {
                            void viewModel.actions.copyAllTrackers();
                        }}
                        isDisabled={viewModel.state.isMutating}
                    >
                        {viewModel.labels.copyAllLabel}
                    </Button>
                </div>
                <span className={TRACKER_UI.selectionSummary}>
                    {viewModel.labels.selectionSummary}
                </span>
            </div>
                ...
                {shell(
                    ...
                    >
                                        ...
                                        {headerGroup.headers.map((header) => {
                                            ...
                                            return (
                                                <th
                                                    key={header.id}
                                                    scope="col"
                                                    className={TRACKER_UI.headerCell}
                                                >
                                                    {isSortable ? (
                                                        <button
                                                            type="button"
                        ...
                        {viewModel.state.contextMenu && (
                            <TrackerContextMenu viewModel={viewModel} />
                        )}
                    </div>,
                )}
            </div>
        ...
        >
            <div className={TRACKER_UI.modalPanel}>
                ...
                {viewModel.state.editor.error ? (
                    <p className={TRACKER_UI.modalError}>
                        {viewModel.state.editor.error}
                    </p>
                ) : null}
            </div>
