...
import { resolveShortcutIntentFromKeyboardEvent } from "@/app/controlPlane/shortcuts";
import { ShortcutIntents } from "@/shared/controlPlane/shortcutVocabulary";
import { getEffectiveTorrentState, isTorrentPausableState } from "@/modules/dashboard/utils/torrentStatus";
...
const { NavigateNextTab, NavigatePreviousTab, NavigateFirstTab, NavigateLastTab } = ShortcutIntents;

const detailTabNavigationIntents = [NavigateNextTab, NavigatePreviousTab, NavigateFirstTab, NavigateLastTab] as const;
...
export const useDetailTabs = ({ inspectorTabCommand, onInspectorTabCommandHandled }: UseDetailTabsParams) => {
    const {
    ...
    }, [inspectorTab, inspectorTabCommand, onInspectorTabCommandHandled, setInspectorTab]);
...
export interface TorrentDetailTabSurfaces {
    ...
    peers: {
        ...
        peers: NonNullable<NonNullable<DashboardDetailViewModel["detailData"]>["peers"]>;
        emptyMessage: string;
...
export const TAB_DEFS: readonly TorrentDetailTabDefinition[] = [
    {
        ...
        render: (surfaces) => (surfaces.general ? createElement(GeneralTab, surfaces.general) : null),
    },
    {
        id: "content",
    ...
    },
    {
        ...
        render: (surfaces) => (surfaces.pieces ? createElement(PiecesTab, surfaces.pieces) : null),
    },
    {
        id: "trackers",
        labelKey: "inspector.tab.trackers",
        isVisible: (surfaces) => surfaces.trackers !== null,
        render: (surfaces) => (surfaces.trackers ? createElement(TrackersTab, surfaces.trackers) : null),
    },
    {
        id: "peers",
    ...
    },
    {
        ...
        render: (surfaces) => (surfaces.speed ? createElement(SpeedTab, surfaces.speed) : null),
    },
] as const;
...
export const useTorrentDetailTabCoordinator = ({
    ...
    const { active, setActive } = useDetailTabs({
    ...
    const trackersViewModel = useTorrentDetailsTrackersViewModel({
        ...
        emptyMessage:
            torrent?.trackers == null ? t("torrent_modal.loading") : t("torrent_modal.trackers.empty_backend"),
        listRef: trackersListRef,
        addTrackers: (nextTrackers) =>
            trackerCommands.torrentId == null
                ? Promise.resolve({ status: "failed" as const })
                : runTrackerMutation(() =>
                      trackerCommands.addTrackers(trackerCommands.torrentId as string | number, nextTrackers),
                  ),
        removeTrackers: (trackerIds) =>
            trackerCommands.torrentId == null
                ? Promise.resolve({ status: "failed" as const })
                : runTrackerMutation(() =>
                      trackerCommands.removeTrackers(trackerCommands.torrentId as string | number, trackerIds),
                  ),
        setTrackerList: (trackerList) =>
            trackerCommands.torrentId == null
                ? Promise.resolve({ status: "failed" as const })
                : runTrackerMutation(() =>
                      trackerCommands.setTrackerList(trackerCommands.torrentId as string | number, trackerList),
                  ),
        reannounce: () =>
            trackerCommands.torrentId == null
                ? Promise.resolve({ status: "failed" as const })
                : runTrackerMutation(() => trackerCommands.reannounce(trackerCommands.torrentId as string | number)),
    });
            ...
            !torrent
                ...
                : {
                      general: {
                          torrent,
                          isDetailFullscreen,
                          sequentialDownloadCapability: viewModel.tabs.general.sequentialDownloadCapability,
                          onTorrentAction: viewModel.tabs.general.handleTorrentAction,
                          onSequentialToggle: viewModel.tabs.general.handleSequentialToggle,
                      ...
                      pieces: {
                          ...
                          sequentialDownload: Boolean(torrent.sequentialDownload),
                      ...
                      peers: {
                          torrentId: torrent.id ?? torrent.hash ?? null,
                          peers: torrent.peers ?? [],
                          emptyMessage:
                              torrent.peers == null
    ...
    const visibleTabDefs = useMemo(
        () => TAB_DEFS.filter((definition) => (definition.isVisible ? definition.isVisible(surfaces) : true)),
        [surfaces],
    );
    ...
    const handleKeyDown = useCallback(
        (event: KeyboardEvent) => {
            ...
            const intent = resolveShortcutIntentFromKeyboardEvent(event, detailTabNavigationIntents);
            if (!intent) {
                return;
            }
            ...
            if (intent === NavigateNextTab) {
                setActive(visibleTabIds[(safeActiveIndex + 1) % visibleTabIds.length]);
                event.preventDefault();
                return;
            }
            if (intent === NavigatePreviousTab) {
                setActive(visibleTabIds[(safeActiveIndex - 1 + visibleTabIds.length) % visibleTabIds.length]);
                event.preventDefault();
                return;
            }
    ...
    const headerActions = useMemo<TorrentDetailHeaderAction[]>(() => {
        if (active === "general" && surfaces.general) {
            ...
            const effectiveState = getEffectiveTorrentState(generalSurface.torrent, generalSurface.optimisticStatus);
            ...
            return [
                {
                    ...
                    ariaLabel: isActiveTorrent ? t("toolbar.pause") : t("toolbar.resume"),
                    tone: isActiveTorrent ? "warning" : "success",
                },
                {
                    icon: RotateCcw,
                    onPress: () => {
                        void generalSurface.onTorrentAction("recheck", generalSurface.torrent);
                    },
                ...
                },
                {
                    icon: Trash2,
                    onPress: () => {
                        void generalSurface.onTorrentAction("remove", generalSurface.torrent);
                    },
        ...
        if (active === "pieces" && surfaces.pieces) {
            ...
            if (torrent && viewModel.tabs.general.sequentialDownloadCapability === "supported") {
                const sequentialEnabled = Boolean(torrent.sequentialDownload);
                const nextSequentialEnabled = !sequentialEnabled;
                actions.push({
                    icon: nextSequentialEnabled ? ListEnd : Shuffle,
                    onPress: () => {
                        void viewModel.tabs.general.handleSequentialToggle(nextSequentialEnabled);
                    },
                    ariaLabel: t(
                        nextSequentialEnabled
                            ? "torrent_modal.piece_map.switch_to_sequential"
                            : "torrent_modal.piece_map.switch_to_random",
                    ),
                    tone: nextSequentialEnabled ? "success" : "neutral",
                });
            }
        ...
        if (active === "trackers" && surfaces.trackers) {
            ...
            if (trackerCommands.torrentId != null && canMutate) {
                actions.push({
                    icon: Plus,
            ...
            if (trackerCommands.torrentId != null && canMutate && trackerState.canEdit) {
                actions.push({
                    icon: Pencil,
            ...
            if (
                ...
                actions.push({
                    ...
                    ariaLabel:
                        trackerState.selectedCount > 1 ? trackerLabels.removeManyLabel : trackerLabels.removeLabel,
                    tone: "danger",
                });
            }
            if (trackerCommands.torrentId != null && canMutate) {
