...
export function WorkspaceModalProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <WorkspaceModalContext.Provider
      value={{ isAddModalOpen, openAddModal, closeAddModal, isSettingsOpen, openSettings, closeSettings }}
    >
      {children}
    </WorkspaceModalContext.Provider>
  );
...
export function useWorkspaceModals(): WorkspaceModalContextValue {
  ...
  if (!context) {
    throw new Error("useWorkspaceModals must be used within a WorkspaceModalProvider");
  }
  return context;
}
