...
import type { LibtorrentPriority } from "../../../core/domain/entities";
...
const PRIORITY_BADGE_BASE = "text-[10px] font-semibold uppercase tracking-[0.2em] px-2 py-0.5 rounded-full";
...
const flattenVisibleNodes = (nodes: FileExplorerNode[], expanded: Record<string, boolean>, depth = 0): VisibleNode[] => {
    ...
    <div ref={containerRef} className="relative h-full overflow-y-auto">
      <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: "relative" }}>
        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
          ...
          const priorityLabel = node.file.priority !== undefined ? PRIORITY_LABELS[node.file.priority] : null;
          const priorityBadgeClass =
            node.file.priority !== undefined ? `${PRIORITY_BADGE_BASE} ${PRIORITY_BADGE_CLASSES[node.file.priority]}` : "";
          return (
            <div
              key={rowKey}
              className="absolute left-0 right-0"
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                  ...
                  {priorityLabel && <span className={priorityBadgeClass}>{priorityLabel}</span>}
                </div>
                ...
                {typeof node.file.progress === "number" && (
                  <span className="text-[11px] font-mono text-foreground/40">{(node.file.progress * 100).toFixed(0)}%</span>
                )}
              </div>
            </div>
          );
