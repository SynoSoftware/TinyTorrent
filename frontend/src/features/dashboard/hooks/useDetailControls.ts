...
export function useDetailControls({
  ...
  const handleFileSelectionChange = useCallback(
    async (indexes: number[], wanted: boolean) => {
      ...
      await runWithRefresh(() => torrentClient.updateFileSelection(detailData.id, indexes, wanted));
    },
  ...
  const handleSequentialToggle = useCallback(
    async (enabled: boolean) => {
      ...
      mutateDetail((current) => ({ ...current, sequentialDownload: enabled }));
  ...
  const handleForceTrackerReannounce = useCallback(async () => {
    ...
    await runWithRefresh(() => reannounceFn(detailData.id), { refreshTorrents: false, refreshStats: false });
