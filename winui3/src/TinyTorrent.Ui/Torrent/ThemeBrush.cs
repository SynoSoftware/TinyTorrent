...
/// the rest of the session as soon as the user switches theme, and because holding one would tie
/// the whole session layer to XAML. This resolves the key each time a cell binds, and the page
/// asks the rows to re-announce their keys when the theme changes, which is what makes the two
/// halves add up to a colour that actually follows the theme.
/// </remarks>
