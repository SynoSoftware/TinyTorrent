...
Options:
  ...
  -LoopTests                                  Run tests in a loop until failure
  -MaxLoopIterations <N>                      Limit loop test iterations (requires -LoopTests)
  -CaptureDumps                               Capture crash dumps during tests (requires procdump)
  -DumpToolPath <path>                        Path to procdump.exe
  -MesonPath <path>                           Override meson executable path
  -NinjaPath <path>                           Override ninja executable path
  -VsWherePath <path>                         Override vswhere.exe path
  -Help                                       Show this help message

Examples:
  .\build.ps1                                 # Build Debug configuration
  .\build.ps1 -Configuration Release          # Build Release configuration
  .\build.ps1 -Clean                          # Clean build Debug
  .\build.ps1 -LoopTests                      # Run tests in loop
"@
    exit 0
}
