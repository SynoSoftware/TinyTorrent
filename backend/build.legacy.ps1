...
Microsoft.PowerShell.Utility\Write-Host "[DEPRECATED] build.legacy.ps1 is deprecated; run './make debug' or './make release' (buildctl.ps1) instead." -ForegroundColor Yellow
Microsoft.PowerShell.Utility\Write-Host "This legacy entrypoint is retained for compatibility but will be removed once the modular flow is stable." -ForegroundColor DarkYellow
...
function Print-Help {
    $helpLines = @(
        "Usage: build.legacy.ps1 [Configuration|clean|help] [options]",
        "",
        "Configurations:",
        ...
        "",
        "Examples:",
        "  ./build.legacy.ps1             # default Debug build (deprecated).",
        "  ./build.legacy.ps1 clean       # clean + Debug rebuild.",
        "  ./build.legacy.ps1 release     # 64-bit release build.",
        "  ./build.legacy.ps1 -ForceVcpkg # rebuild vcpkg packages first.",
        "",
        "This legacy script bootstraps vcpkg, runs Meson/Ninja, and optionally runs",
        "the doctest suite. Pass -SkipTests to avoid long runtimes.",
        "Use './make debug' or './make release' via buildctl.ps1 for the new workflow."
    )
