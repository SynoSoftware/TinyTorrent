...
void run_ws_client(WsTestContext &context, std::string const &url,
  ...
  if (!headers.empty()) {
    conn = mg_ws_connect(&mgr, url.c_str(), ws_client_handler, &context, "%s",
                         headers.c_str());
  } else {
...
struct FilesystemOverride {
  explicit FilesystemOverride(
      ...
      : previous_entries(entries ? tt::rpc::filesystem::set_directory_entries_handler(entries)
                                : tt::rpc::filesystem::DirectoryEntriesFn{}),
        previous_exists(exists ? tt::rpc::filesystem::set_path_exists_handler(exists)
                               : tt::rpc::filesystem::PathCheckFn{}),
        previous_is_directory(
            is_dir ? tt::rpc::filesystem::set_is_directory_handler(is_dir)
                   : tt::rpc::filesystem::PathCheckFn{}),
        previous_space(space ? tt::rpc::filesystem::set_space_query_handler(space)
  ...
  ~FilesystemOverride() {
    if (entries_overridden) {
      tt::rpc::filesystem::set_directory_entries_handler(previous_entries);
    }
    if (exists_overridden) {
      tt::rpc::filesystem::set_path_exists_handler(previous_exists);
    }
    if (is_directory_overridden) {
      tt::rpc::filesystem::set_is_directory_handler(previous_is_directory);
    }
    if (space_overridden) {
...
TEST_CASE("fs-browse honors mocked directory entries") {
  ...
  auto response =
      send_rpc_request(R"({"method":"fs-browse","arguments":{"path":"C:\\fake"}})");
...
TEST_CASE("fs-space reports mocked metrics") {
  ...
  auto response =
      send_rpc_request(R"({"method":"fs-space","arguments":{"path":"C:\\fake"}})");
  ResponseView view{response};
  auto *arguments = view.arguments();
  REQUIRE(arguments != nullptr);
