...
bool PersistenceManager::persist_session_stats(SessionStatistics const &stats) {
  ...
  bool success = true;
  success = success && database_->set_setting("secondsActive",
                                             std::to_string(stats.seconds_active));
  success = success && database_->set_setting("uploadedBytes",
                                             std::to_string(stats.uploaded_bytes));
  success = success && database_->set_setting("downloadedBytes",
...
bool PersistenceManager::upsert_torrent(storage::PersistedTorrent const &torrent) {
  if (!is_valid()) {
    return false;
  }
...
bool PersistenceManager::update_metadata(std::string const &hash,
                                         std::string const &path,
...
bool PersistenceManager::update_resume_data(std::string const &hash,
...
std::uint64_t PersistenceManager::read_uint64_setting(std::string const &key) const {
  if (!is_valid()) {
    return 0;
  }
