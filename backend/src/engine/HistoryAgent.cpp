...
void HistoryAgent::flush_if_due(std::chrono::steady_clock::time_point now,
  ...
  if (bucket_timestamp == 0) {
    bucket_timestamp =
        align_to_history_interval(std::chrono::system_clock::now(),
  ...
  schedule_task_async([this, bucket_timestamp, down_bytes, up_bytes]() {
    if (!database_ || !database_->is_valid()) {
      return;
    }
...
void HistoryAgent::perform_retention(
  ...
  schedule_task_async([this, cutoff]() {
    if (!database_) {
      return;
    }
...
std::vector<HistoryBucket> HistoryAgent::query(std::int64_t start,
                                               std::int64_t end,
                                               std::int64_t step) {
  if (!database_ || !database_->is_valid()) {
    return {};
  }
  auto future = schedule_task_async([this, start, end, step]() {
    if (!database_ || !database_->is_valid()) {
    ...
    for (auto const &entry : entries) {
      result.push_back(HistoryBucket{
          entry.timestamp, entry.total_down, entry.total_up, entry.peak_down,
          entry.peak_up});
    }
    return result;
  });
...
bool HistoryAgent::clear(std::optional<std::int64_t> older_than) {
  ...
  if (older_than) {
    auto future = schedule_task_async([this, cutoff = *older_than]() {
      if (!database_) {
        return false;
      }
...
HistoryConfig HistoryAgent::config() const { return config_; }
...
void HistoryAgent::update_config(HistoryConfig config, bool flush_after,
  ...
  config_.interval_seconds = normalize_history_interval(config.interval_seconds);
