...
void TorrentManager::enqueue_task(std::function<void()> task) {
  {
    ...
    while (tasks_.size() >= kMaxPendingTasks) {
      TT_LOG_INFO("task queue maxed out ({}); waiting for engine to catch up", tasks_.size());
...
TorrentManager::SnapshotBuildResult TorrentManager::build_snapshot(
    SnapshotBuildCallbacks const &callbacks) {
  ...
  for (auto const &handle : handles) {
    ...
    std::uint64_t revision = callbacks.ensure_revision
                                 ? callbacks.ensure_revision(id)
                                 : 0;
  ...
  snapshot->active_torrent_count =
      snapshot->torrent_count > paused_count ? snapshot->torrent_count - paused_count
                                            : 0;
...
void TorrentManager::process_alerts() {
  ...
  for (auto const *alert : alert_buffer) {
    if (auto *finished =
            libtorrent::alert_cast<libtorrent::torrent_finished_alert>(alert)) {
      ...
      if (callbacks_.on_storage_moved_failed) {
        callbacks_.on_storage_moved_failed(*storage_failed);
               ...
               libtorrent::alert_cast<libtorrent::fastresume_rejected_alert>(alert)) {
...
std::vector<libtorrent::torrent_handle> TorrentManager::handles_for_ids(
...
std::vector<char> TorrentManager::write_session_params(
...
std::optional<int> TorrentManager::id_for_hash(
    libtorrent::sha1_hash const &hash) const {
  if (!hash_is_nonzero(hash)) {
    return std::nullopt;
  }
  auto it = hash_to_id_.find(hash);
...
std::optional<libtorrent::torrent_handle> TorrentManager::handle_for_id(int id) const {
  if (id <= 0 || !session_) {
    return std::nullopt;
  }
...
std::optional<TorrentSnapshot> TorrentManager::cached_snapshot(
    int id, std::uint64_t revision) const {
  if (id <= 0) {
    return std::nullopt;
  }
...
std::vector<int> TorrentManager::purge_missing_ids(
...
void TorrentManager::queue_pending_move(std::string const &hash,
