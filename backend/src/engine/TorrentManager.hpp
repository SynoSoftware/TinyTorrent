...
public:
  struct AlertCallbacks {
    std::function<void(libtorrent::torrent_handle const &, libtorrent::torrent_status const &)>
        on_torrent_finished;
    std::function<std::filesystem::path(std::string const &)> metadata_file_path;
    std::function<void(std::string const &, std::filesystem::path const &,
                       std::vector<std::uint8_t> const &)>
        on_metadata_persisted;
    std::function<void(std::string const &, libtorrent::add_torrent_params const &)>
        on_resume_data;
    std::function<void(std::string const &)> on_resume_hash_completed;
    std::function<void()> extend_resume_deadline;
    std::function<void(std::vector<libtorrent::torrent_status> const &)> on_state_update;
    std::function<void(libtorrent::listen_succeeded_alert const &)> on_listen_succeeded;
    std::function<void(libtorrent::listen_failed_alert const &)> on_listen_failed;
    std::function<void(libtorrent::file_error_alert const &)> on_file_error;
    std::function<void(libtorrent::tracker_error_alert const &)> on_tracker_error;
    std::function<void(libtorrent::portmap_error_alert const &)> on_portmap_error;
    std::function<void(libtorrent::storage_moved_alert const &)> on_storage_moved;
    std::function<void(libtorrent::storage_moved_failed_alert const &)> on_storage_moved_failed;
    std::function<void(libtorrent::fastresume_rejected_alert const &)>
        on_fastresume_rejected;
  };

  struct SnapshotBuildCallbacks {
    std::function<TorrentSnapshot(int, libtorrent::torrent_status const &, std::uint64_t)>
        build_snapshot_entry;
    std::function<void(int, libtorrent::torrent_handle const &,
                       libtorrent::torrent_status const &)>
        on_torrent_visit;
    std::function<std::vector<std::string>(int, std::string const &)> labels_for_torrent;
  ...
  std::optional<TorrentSnapshot> cached_snapshot(int id,
                                                std::uint64_t revision) const;
  std::vector<int> purge_missing_ids(
  ...
  std::vector<libtorrent::torrent_handle> handles_for_ids(
      std::vector<int> const &ids) const;
  void set_ip_filter(libtorrent::ip_filter const &filter);
  void remove_torrent(libtorrent::torrent_handle const &handle, bool delete_data);
  std::vector<char> write_session_params(
      libtorrent::save_state_flags_t mode) const;
