...
#define MG_ENABLE_TCPIP 1 // Enable built-in TCP/IP stack
    ...
    !defined(MG_ENABLE_DRIVER_STM32H)
...
#define closesocket(x) soc_close(x)
    ...
    // In order to enable BSD support in NetxDuo, do the following (assuming
    // Cube):
    ...
    //   Project -> Properties -> C/C++ -> Settings -> MCU Compiler ->
    //   Preprocessor
...
#define getpeername(a, b, c) mg_getpeername((a), (b), (c))
    ...
    static inline int mg_getsockname(MG_SOCKET_TYPE fd, void *buf,
                                     socklen_t *len)
    {
    ...
    static inline int mg_getpeername(MG_SOCKET_TYPE fd, void *buf,
                                     socklen_t *len)
    {
        (void)fd, (void)buf, (void)len;
        return 0;
...
#define MG_ENABLE_CHACHA20 1 // When set to 0, GCM is used. For MG_TLS_BUILTIN
    ...
    // 32-bit timestamp in milliseconds since connection init time.
    //
    ...
    struct mg_profitem
    {
        ...
        uint32_t
            timestamp; // Milliseconds since connection creation (MG_EV_OPEN)
    };
...
#define MG_TIMER_AUTODELETE 8  // mg_free() timer when done
    ...
    struct mg_fs
    {
        ...
        void (*ls)(const char *path, void (*fn)(const char *, void *),
                   void *); // List directory entries: call fn(file_name,
...
#define LIST_DELETE(type_, head_, elem_)                                       \
    ...
    enum
    {
        ...
        MG_EV_MQTT_OPEN, // MQTT CONNACK received        int
                         // *connack_status_code
    ...
    struct mg_connection
    {
        ...
        unsigned
            is_arplooking : 1; // Non-blocking ARP resolution is in progress
...
#define MG_UECC_SUPPORT_COMPRESSED_POINT 1
    ...
    extern "C"
    {
        ...
        could not be generated. The filled-in values should be either truly
        random, or from a cryptographically-secure PRNG.

        A correctly functioning RNG function must be set (using
        mg_uecc_set_rng()) before calling mg_uecc_make_key() or mg_uecc_sign().
        ...
        Linux, or OS X. If you are building on another POSIX-compliant system
        that supports /dev/random or /dev/urandom, you can define MG_UECC_POSIX
        to use the predefined RNG. For embedded platforms there is no predefined
        ...
        */
        ...
        platforms), this must be called before mg_uecc_make_key() or
        mg_uecc_sign() are used.

        Inputs:
            rng_function - The function that will be used to generate random
        bytes.
        */
        ...
        */
        ...
        least 2 * the curve size (in bytes) long. For example, if the curve is
        secp256r1, public_key must be 64 bytes long. private_key - Will be
        filled in with the private key. Must be as long as the curve order; this
        is typically the same as the curve size, except for secp160r1. For
        example, if the curve is secp256r1, private_key must be 32 bytes long.
        ...
        */
        ...
        result of mg_uecc_shared_secret() before using it for symmetric
        encryption or HMAC.
        ...
        same size as the curve size; for example, if the curve is secp256r1,
        ...
        */
        ...
        Outputs:
            compressed - Will be filled in with the compressed public key. Must
        be at least (curve size + 1) bytes long; for example, if the curve is
        ...
        */
        ...
        using any other uECC functions. However, you may wish to avoid spending
        CPU time computing a shared secret or verifying a signature using an
        invalid public key.
        ...
        */
        ...
        recommended) and pass it in to this function along with your private
        key.
        ...
        */
        ...
        hash computations; each time a new hash is computed, init_hash() will be
        called, followed by one or more calls to update_hash(), and finally a
        call to finish_hash() to produce the resulting hash.
        ...
        */
        typedef struct MG_UECC_HashContext
        {
            ...
            unsigned block_size; /* Hash function block size in bytes, eg 64 for
                                    SHA-256. */
            unsigned result_size; /* Hash function result size in bytes, eg 32
                                     for SHA-256. */
        ...
        deterministic algorithm (see RFC 6979). You do not need to set the RNG
        using mg_uecc_set_rng() before calling this function; however, if the
        RNG is defined it will improve resistance to side-channel attacks.

        Usage: Compute a hash of the data you wish to sign (SHA-2 is
        recommended) and pass it to this function along with your private key
        and a hash context. Note that the message_hash does not need to be
        computed with the same hash function used by hash_context.
        ...
        */
        ...
        signer and pass it to this function along with the signer's public key
...
#define PORTABLE_8439_DECL
        ...
        /*
         ...
         encrypted messages. This does assume you never reuse a nonce+key pair
         and, if possible, carefully pick your associated data.
        */
...
#define RFC_8439_NONCE_SIZE (12)
        ...
        /*
            ...
            input:
                ...
                - nonce: RFC_8439_NONCE_SIZE bytes that should never be repeated
                    for the same key. A counter or a pseudo-random value are
           fine.
           ...
           overlap with cipher_text pointer
            ...
            output:
                - cipher_text: encrypted plain_text with a tag appended. Make
           sure to allocate at least plain_text_size + RFC_8439_TAG_SIZE
        ...
        */
        ...
        /*
            Decrypt/unseal cipher text given the right key, nonce, and
           additional data.
...
#define MQTT_PROP_SHARED_SUBSCRIPTION_AVAILABLE 0x2A
    ...
    struct mg_mqtt_opts
    {
        ...
        struct mg_mqtt_prop
            *will_props;       // Valid only for CONNECT packet (MQTT5)
    ...
    // If `resolved` is true, then `addr` contains resolved IPv4 or IPV6
    // address.
...
#define MG_WIFI_BAND_5G 1
    ...
    enum
    {
        ...
        MG_TCPIP_EV_DHCP_SNTP, // DHCP SNTP assignment           uint32_t
                               // *ipaddr
        ...
        MG_TCPIP_EV_WIFI_CONNECT_ERR, // Wi-Fi connect has failed       driver
                                      // and chip specific
...
#include "Driver_ETH_PHY.h" // keep this include
    ...
    struct mg_tcpip_driver_cyw_data
    {
        ...
        bool hs; // use chip "high-speed" mode; otherwise SPI CPOL0 CPHA0
                 // (DS 4.2.3 Table 6)
    };
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                              \
    ...
    struct mg_tcpip_driver_rw612_data
    {
        ...
        // follows: MDC_freq = P_clock / ((mdc_cr + 1) * 2), where P_clock is
        // the peripheral bus clock. IEEE802.3 clause 22 defines a minimum of 10
        // ns for the hold time on the MDIO output. Depending on the host bus
        // frequency, the setting may need to be increased.
        int mdc_cr;
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                              \
    ...
    struct mg_tcpip_sdio
    {
        ...
        // SDIO extended transaction: write or read len bytes, using blksz
        // blocks
        bool (*xfr)(void *, bool write, uint32_t arg, uint16_t blksz,
                    uint32_t *, uint32_t len, uint32_t *r);
    };

    // API with driver (e.g.: cyw.c)
    // Once the hardware controller has been initialized:
    // - Init card: selects the card, sets F0 block size, sets bus width and
    // speed
    ...
    // - Requesting a read transfer > blocksize means block transfer will be
    // used.
    // - Drivers must have room to accomodate a whole block transfer, see sdio.c
    // - Transfers of > 1 byte --> (uint32_t *) data. 1-byte --> (uint8_t *)
    // data
    bool mg_sdio_transfer(struct mg_tcpip_sdio *sdio, bool write,
                          unsigned int f, uint32_t addr, void *data,
                          uint32_t len);
    ...
    struct mg_tcpip_driver_stm32f_data
    {
        ...
        //    20-35 MHz     HCLK/16        2
        //    35-60 MHz     HCLK/26        3
        //    150-216 MHz   HCLK/102       4  <-- value for Nucleo-F* on max
        //    speed 216-310 MHz   HCLK/124       5 110, 111 Reserved
